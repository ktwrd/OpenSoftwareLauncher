...
namespace OpenSoftwareLauncher.Server.Controllers.Admin.User
{
    ...
    public class UserLicenseController : Controller
    {
        ...
        {
            var tokenAccount = MainClass.GetService<MongoAccountManager>()?.GetAccount(token, false);
            ...
            if (tokenAccount != null)
                MainClass.GetService<AuditLogManager>()?.Create(new AccountLicenseUpdateEntryData(account, previous, account.Licenses.ToArray()), tokenAccount).Wait();
